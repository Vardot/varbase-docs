...
{% hint style="info" %}
**Varbase E2E replaces Webship-js.** The `varbase_project` 10.1.x branch requires [`@vardot/varbase-e2e`](https://www.npmjs.com/package/@vardot/varbase-e2e) `^2`. Old `webship-js` step paths in a project's `cucumber.js` no longer resolve; see [Moving From Webship-js](#moving-from-webship-js).
{% endhint %}
