...
{% hint style="info" %}
**Varbase E2E replaces Webship-js.** From `varbase_project` 11.0.x, 10.1.x and 9.2.x, the suite runs on [`@vardot/varbase-e2e`](https://www.npmjs.com/package/@vardot/varbase-e2e) `^2`. Old `webship-js` step paths in a project's `cucumber.js` no longer resolve; see [Moving From Webship-js](#moving-from-webship-js).
{% endhint %}
